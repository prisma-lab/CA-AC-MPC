...
class _NoOpLQRStepFn(Function):
    """Module-level autograd Function for the differentiable (no-op forward) LQR step.

    Defined at module level (single class) to avoid PyTorch accumulating
    a new Function subclass on every call -- which was the source of the
    ~0.5 MB / iter memory leak.
    """

    @staticmethod
    def forward(ctx, x_init, C, c, F, f, current_x, current_u,
                n_state, n_ctrl, T, u_lower, u_upper, back_eps):
    ...
    def backward(ctx, dl_dx, dl_du):
        ...
        r = torch.stack([
            torch.cat((dl_dx[t], dl_du[t]), 1) for t in range(T)
        ])
        ...
        # Gradients for: x_init, C, c, F, f, current_x, current_u,
        #                 n_state, n_ctrl, T, u_lower, u_upper, back_eps
        return dx_init, dC, dc, dF, df, None, None, None, None, None, None, None, None
...
def LQRStep(n_state,
    ...
    # For the differentiable (no-op) pass, use module-level class to avoid
    # accumulating new Function subclasses (memory leak).
    if no_op_forward:
        def apply_fn(x_init, C, c, F, f=None):
            if f is None:
                f_arg = _EMPTY_TENSOR
            else:
                f_arg = f
            return _NoOpLQRStepFn.apply(
                x_init, C, c, F, f_arg, current_x, current_u,
                n_state, n_ctrl, T, u_lower, u_upper, back_eps)
        return apply_fn

    # For the iterative (non-differentiable) pass, use closure-based class.
    # This path runs under torch.no_grad() so classes are cleaned up.

    # @profile
    def lqr_backward(C, c, F, f):
        n_batch = C.size(1)
    ...
    # @profile
    def lqr_forward(x_init, C, c, F, f, Ks, ks):
        x = current_x
        u = current_u
        n_batch = C.size(1)
    ...
    class LQRStepFn(Function):
        ...
        def forward(ctx, x_init, C, c, F, f=None):
            ...
            Ks, ks, n_total_qp_iter = lqr_backward(C, c_back, F, f_back)
