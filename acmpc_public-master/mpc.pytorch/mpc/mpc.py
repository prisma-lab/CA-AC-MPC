...
from .lqr_step import LQRStep, _NoOpLQRStepFn, _EMPTY_TENSOR
...
class SlewRateCost(Module):
    """Add a slew-rate penalty to costs."""
...
class MPC(Module):
    ...
    Optional Args:
        ...
        n_batch: Explicit batch size override.
    ...
    def forward(self, x_init, cost, dx):
        ...
        n_not_improved = 0
        # Iteration loop: pure optimization, no autograd graph needed.
        with torch.no_grad():
            for i in range(self.lqr_iter):
                u = util.detach_maybe(u).detach()
                ...
                if best is None:
                    best = {
                        ...
                        'u': list(torch.split(u, 1, dim=1)),
                        'costs': costs.clone(),
                        'full_du_norm': full_du_norm.clone(),
                    }
                else:
        ...
        if self.detach_unconverged:
            if max(full_du_norm) > self.eps:
                ...
                Ix = I.unsqueeze(0).unsqueeze(2).expand_as(x).to(dtype=x.dtype)
                Iu = I.unsqueeze(0).unsqueeze(2).expand_as(u).to(dtype=u.dtype)
                x_detached = x.detach()
                u_detached = u.detach()
                x = x*Ix + x_detached*(1.-Ix)
                u = u*Iu + u_detached*(1.-Iu)
                del x_detached, u_detached, Ix, Iu
...
""")
            ...
            for t in range(self.T):
                ...
                hessian = list()
                for v_i in range(n_tau_dims):
                    # Only retain_graph if not the last column
                    retain = (v_i < n_tau_dims - 1)
                    hessian.append(
    ...
    def linearize_dynamics(self, x, u, dynamics, diff):
        ...
        if self.grad_method == GradMethods.ANALYTIC:
            _u = u[:-1].view(-1, self.n_ctrl).detach().requires_grad_(True)
            _x = x[:-1].contiguous().view(-1, self.n_state).detach().requires_grad_(True)
            ...
            for t in range(self.T):
                if t < self.T-1:
                    xt = x[t].detach().requires_grad_(True)
                    ut = u[t].detach().requires_grad_(True)
